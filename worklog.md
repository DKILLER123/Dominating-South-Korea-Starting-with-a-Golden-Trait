...
**Tree state (cycle 12):** 38 chapters (`ch001`–`ch038`, next slot `ch039`) · 47 XML documents ·
100 payload entries · 97 manifest items · 44 spine entries · 43 NCX navPoints · 46 nav list items ·
29 images (`id-01`…`id-29` — `id-28` Mina, `id-29` Taeyeon, web-sourced press photographs,
736×920; id ceiling 29, next manifest id `id-30`) · 20 embedded WOFF faces · 20 character cards
(+ Mina, + Taeyeon; + Lee Yoon-hyang from cycle 11) · 20 introduction cards · 57 glossary cards
(+ peninsula’s first heartthrob) · 0 structural errors (validate_tree 44/44 + in-archive asserts
at build).

**Package (cycle-12 ship):** `Dominating_South_Korea_Starting_with_a_Golden_Trait.epub` ·
100 entries · 5,522,418 B · sha256 `e33b5cd81801c29677402c4987eb43707ce65ba563009236815574de83f86fd9` ·
mimetype STORED first · testzip clean · in-archive asserts at build (100 entries; 3 new chapters +
2 new portraits present; front matter wired). Prior ships remain snapshotted: `extracted-v10/`
(cycle 10, 91 entries), `extracted/` (v1, 43 entries) — untouched per the snapshot convention;
newer archives are snapshotted to new named folders on the reader’s request.
...
**Canon pins:** see §1. The book now ends at ch038: the lucky-fan plan is in motion — the
account is being farmed, the photo pipeline and the smiling-face-emoji signal are set for two
days out, and Bae Suzy sits with the question she has not conceded to anyone (first place
conceded, second place not). The ten-member group twice has been named (debut “this year”);
Mina is filed as the collision, and Taeyeon has delivered her four courses of captainship and
paused the consultation at the meat. Bae Do-yoon’s blank account carries the nickname The
Peninsula’s First Heartthrob; the follow list holds two names — Bae Suzy (followed just before
the meeting) and Irene (followed a very long time ago; “I am her fan”). Lee Boo-jin’s
...
comment section that declares; “this is a sickness — so it has to be treated”); his name is
still unsaid. Jessica’s first command stands (five clauses, executed by presence), the
obedience terms carry their single exception ("of course — except for him"); Bae Do-yoon
carries two purple entries, one blue, one gold, and the white-rank `Combat Beginner` earned in
ch019.
...
**Next cycle (when the reader supplies Chapter 39+):** next numeric image manifest id is
`id-30`. Save `raws/ch039_raw.txt` first → recon greps → draft → wire-up (manifest `ch039`, NCX
`num_44`/playOrder 44, nav `<li>` 47, cover `#chapters-stamp` and both OPF descriptions →
`Chapters 1–39`, glossary header/footer + cards as earned) → all of SKILL §9 → worklog §8 + §0 →
