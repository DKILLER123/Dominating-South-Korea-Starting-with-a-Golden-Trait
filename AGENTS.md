...
## Current state (cycle 12, 2026-09-15)

Tree: 38 chapters (`ch001`–`ch038`, next slot `ch039`) · 100 payload entries · 97 manifest
items · 44 spine entries · 43 NCX navPoints · 46 nav list items · 20 WOFF faces · 29 images
...
photographic-real) · 20 character cards (+ Lee Yoon-hyang, + Mina, + Taeyeon) · 20 introduction
cards · 57 glossary cards (+ peninsula’s first heartthrob). Package built and verified: 100
entries · 5,522,418 B · sha256 `e33b5cd8…` — in-archive asserts at build. Delivered archive
snapshots: `extracted/` (v1, 43 entries) and `extracted-v10/` (91 entries). All gates green
...
ch032–037 (pullquote re-presentation, speech→contract record, raw-faithful refrains —
cycle-11/12 entries). Next chapter slot `ch039` (NCX `num_44`/playOrder 44, nav li 47; next
